...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (lastUserMsg?.role === 'user') {
      ...
      // Upload raw files to user-files bucket and register initial metadata immediately
          ...
          lastUserMsg.attachments.map(async (att) => {
            ...
            if (payload && !finalStoragePath) {
              try {
                ...
                console.warn('[Chat API] File storage warning:', upErr);
                finalStoragePath = `${profileUid}/${att.id}-${att.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`;
                att.storagePath = finalStoragePath;
              }
            }

            // Immediately register the file in uploaded_files table
            try {
              ...
              console.warn('[Chat API] Initial file metadata record warning:', initMetaErr);
            }
          })
        );
      }
