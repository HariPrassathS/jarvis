...
import { uploadUserFile, getSignedFileUrl, deleteUserFiles } from '../src/lib/files/storage';
import { insertUploadedFile, getUploadedFiles, deleteUploadedFilesByProfile } from '../src/lib/files/metadata';
import { recallUploadedFiles } from '../src/lib/tools/files';
import { processUploadedFile } from '../src/lib/document/extractor';
import { routeChat } from '../src/lib/llm/router';
...
async function runTests() {
  ...
  try {
    ...
    const imgFileId = `file-${Date.now()}-img1`;
    ...
    assert(uploadRes.storagePath.startsWith(`${TEST_UID}/`), `File path scoped to operator firebase_uid: ${uploadRes.storagePath}`);
    const signedUrl = await getSignedFileUrl(uploadRes.storagePath, 3600);
    assert(!!signedUrl, 'Generated secure signed URL for operator access');
    ...
    const docFileId = `file-${Date.now()}-doc1`;
    ...
    const fileRec2 = await insertUploadedFile({
      profile_id: TEST_PROFILE_ID,
      storage_path: docUploadRes.storagePath,
      file_type: 'document',
    ...
    assert(recallResult1.includes('arc_reactor_mark5_schematic.png'), 'Found files matching query "Arc Reactor blueprint"');
    assert(
      recallResult1.includes('3.2 gigawatt'),
    ...
    assert(recallResult2.includes('security_audit_report.pdf'), 'Correctly filtered files by type = "document"');
    assert(!recallResult2.includes('arc_reactor_mark5_schematic.png'), 'Image omitted when file_type = "document"');

    // 4C: Time filter = 'today'
    const recallResult3 = await recallUploadedFiles(TEST_PROFILE_ID, {
      time_range: 'today',
    });
    assert(recallResult3.includes('arc_reactor_mark5_schematic.png') && recallResult3.includes('security_audit_report.pdf'), 'Timeframe filter "today" retrieved all recently uploaded files');
    ...
    assert(otherUserRecall.includes('No stored files') || otherUserRecall.includes('No past uploaded'), 'Unrelated operator CANNOT retrieve another operator\'s files');
    ...
    // Tool returns cached AI description
    ...
    const llmResponse = await routeChat({
      messages: llmPrompt,
      taskType: 'quick_chat',
    });
    ...
    const storageDeleteRes = await deleteUserFiles(TEST_UID);
    assert(storageDeleteRes.deletedCount >= 2, `Purged ${storageDeleteRes.deletedCount} raw files from Supabase Storage bucket`);

    // Delete rows from metadata table
    const dbDeleteRes = await deleteUploadedFilesByProfile(TEST_PROFILE_ID);
    assert(dbDeleteRes.deletedCount >= 2, `Purged ${dbDeleteRes.deletedCount} metadata rows from uploaded_files table`);
